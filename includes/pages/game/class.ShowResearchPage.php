...
class ShowResearchPage extends AbstractPage
{
	...
	private function CancelBuildingFromQueue()
	{
		...
		if($PLANET['id'] == $USER['b_tech_planet'])
		{
			...
			if(isset($costRessources[922])) { $GLOBALS['DATABASE']->query("UPDATE ".USERS." set antimatter = antimatter + ".$costRessources[922]." WHERE `id` = '".$USER['id']."';"); }
		} else {
			...
			if(isset($costRessources[922])) { $SQL	.= $resource[922]." = ".$resource[922]." + ".$costRessources[922].", "; }
  ...
  if($this->bOnInsert==FALSE)
  {
   ...
   if($this->build_anz>=1 )
   {
	...
	private function DoAddBuildingToQueue($Element, $AddMode = true)	{
		...
		if(Config::get('max_elements_tech') != 0 && Config::get('max_elements_tech') + $premium_stage <= $ActualCount)
			return false;
